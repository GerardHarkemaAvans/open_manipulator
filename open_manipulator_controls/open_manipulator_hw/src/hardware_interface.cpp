...
void HardwareInterface::read()
{
  ...
  for(uint8_t index = 0; index < id_cnt; index++)
  {
    ...
    if (strcmp(name_array[index].c_str(), "gripper") == 0)
      ...
      joints_[id_array[index]-1].position *= -0.015; // Changed by Gerard
...
//        dynamixel_position[id_cnt] = dxl_wb_->convertRadian2Value((uint8_t)dxl.second, joints_[(uint8_t)dxl.second-1].position_command * 150.0);
        dynamixel_position[id_cnt] = dxl_wb_->convertRadian2Value((uint8_t)dxl.second, joints_[(uint8_t)dxl.second-1].position_command / -0.015); // Changed by gerard
      id_cnt ++;
    }
